...
configureSpinnaker() {
    enabled=$(kubectl --kubeconfig $K3S_CONFIG -n spin exec -it spin-spinnaker-halyard-0 -- bash -c "hal config security authn oauth2 --no-validate -q -o json" | jq -r -j '.enabled')
    if [ "$enabled" = "true" ]
    then
